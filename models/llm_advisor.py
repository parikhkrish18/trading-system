...
def _build_system_prompt() -> str:
    ...
    return (
        ...
        "window, not a slow-grind long-term story. Take-profit targets are "
        f"sized as a multiple of the stock's own ATR (atr_pct on each "
        f"candidate: its 14-day Average True Range as a fraction of price) "
        f"scaled to this horizon -- normally {tp_min_mult:g}x-{tp_max_mult:g}x "
        "of that horizon-scaled ATR. A stock with no realistic path to a "
        f"{tp_min_mult:g}x-ATR move in about a week is a weak pick here even "
        "if the raw forecast number looks fine; a stock whose setup "
        "(momentum, breakout, catalyst, news) genuinely supports reaching "
        f"the top of that band ({tp_max_mult:g}x ATR) within the week is a "
        "stronger one. Weigh confidence accordingly -- this is exactly the "
        "kind of judgment the quant model alone can't make.\n\n"
        ...
        f"{tp_min_mult:g}x-{tp_max_mult:g}x ATR range this week\n"
        ...
        f"what the existing ATR-relative formula ({tp_min_mult:g}x-{tp_max_mult:g}x "
        "this stock's own horizon-scaled ATR, from atr_pct) would set for "
        "that stock as a reference point; you may suggest something "
        "different if you have good reason, but a similar magnitude is "
        "usually right -- a target far outside that band on a normal setup "
        "usually means the horizon is wrong for this stock, not that the "
        "target should be.\n\n"
